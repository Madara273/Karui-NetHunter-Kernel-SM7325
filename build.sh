...
echo -e "${GREEN}1.👉 NEUTRON CLANG (God Mode)${NC}"
...
case "$COMPILER_CHOICE" in
	1)
		...
		;;
	*)
		echo -e "${RED}Invalid choice. Defaulting to Neutron Clang...${NC}"
		export PATH="$(pwd)/../Madara-Built/NEUTRON/bin:$PATH"
		export CLANG_TRIPLE="aarch64-linux-gnu-"
		export CROSS_COMPILE="aarch64-linux-gnu-"
		export CROSS_COMPILE_ARM32="arm-linux-gnueabi-"
...
# ---- Clone Anykernel3 ----
clone_anykernel3(){
	echo -e "${YELLOW}Cloning AnyKernel3 (Karui-Kernel)...${NC}"
	git clone --depth=1 https://github.com/Madara273/AnyKernel3.git -b Karui-Kernel "$AK3_PATH" &&
	echo -e "${GREEN}Clone successful.${NC}" || echo -e "${RED}Clone failed.${NC}"
}
...
generate_flashable() {
	...
	FLASHABLE_ZIP="Karui-$CLEAN_TIME.zip"
	SIGNED_ZIP="Karui-$CLEAN_TIME-signed.zip"
...
display_kernel_version_info() {
	...
	LAHAINA-QGKI_DEFCONFIG=$(find . -name 'lanaina-qgki_defconfig' -print -quit)
...
compile_kernel() {
	CURRENT_COMPILER="Neutron Clang"
